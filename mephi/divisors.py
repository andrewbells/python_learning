    ...
    for x in range(a, b + 1):
        ...
        for y in range(2, x + 1):
            if x % y == 0 and x != y:
                ...
                divisors_dict[x] = divisors_list
        '''below i added c functionality, replacing function max_num_least(a, b)'''
        if l >= num_divisors:
            l = num_divisors
            v = x
        if r <= num_divisors:
...
def max_num_least(a, b):
    ...
    print ('The max value from the interval, which has least amount of divisors, besides 0 and self is %d' % max_val)
    return max_val
